    ...
    const analyzedData = useMemo((): AnalyzedService[] => {
        return services
            ...
            .map(s => {
                ...
                // Use defined cost if available, otherwise 0
                const cost = s.defaultCost || 0; 
                        ...
                        <tbody className="divide-y" style={{ borderColor: 'var(--border-color)'}}>
                            {analyzedData.map(item => (
                                <tr key={item.id} className="hover:bg-black/10">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm text-red-400">{(item.defaultCost || 0).toLocaleString()}</td>
