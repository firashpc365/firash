...
import { DownloadIcon, SparkleIcon, EditIcon, TrashIcon, EyeIcon, DocumentTextIcon, CheckCircleIcon } from '../common/icons';
...
export const QuotationGenerator: React.FC<QuotationGeneratorProps> = ({
  ...
  // Track which items are included in the quote (true by default)
  ...
  // Derived filtered list for preview and PDF
  ...
  const handleSaveChanges = () => {
      // We essentially save the filtered list as the 'official' quotation list on the event
  ...
  const handleSaveTemplate = () => {
    ...
    onAddTemplate({ 
        templateName, 
        details, 
        lineItems: filteredLineItems // Save only the currently selected items
    });
    setTemplateName('');
    setSuccess(`Template "${templateName}" saved with ${filteredLineItems.length} items.`);
  };
  ...
  const handleRemoveItem = (itemId: string) => {
    // Completely remove from the list
                ...
                <div className="flex-grow lg:grid lg:grid-cols-12 lg:gap-8 overflow-hidden relative">
                    ...
                    <div className={`h-full overflow-y-auto pr-2 custom-scrollbar lg:col-span-4 space-y-6 pb-10 ${mobileTab === 'editor' ? 'block' : 'hidden lg:block'}`}>
                        ...
                        <div className="p-4 rounded-xl bg-black/20 border border-white/5">
                            <div className="flex justify-between items-center mb-3">
                                <h3 className="font-bold text-base flex items-center gap-2" style={{ color: 'var(--text-primary-color)'}}>
                                    <DocumentTextIcon className="h-5 w-5 text-[var(--primary-accent-color)]"/> Quotation Items
                                </h3>
                                <span className="text-xs text-[var(--text-secondary-color)]">{filteredLineItems.length} included</span>
                            </div>
                        ...
                        <div className="space-y-5">
                            ...
                            <div className="border-t pt-4 border-white/10 bg-black/20 p-4 rounded-xl">
                                ...
                                <p className="text-xs text-[var(--text-secondary-color)] mb-3">Save current selection and styling as a reusable template.</p>
                                ...
                                <div className="mt-4 pt-4 border-t border-white/10">
                                    <label className="block text-xs font-medium mb-1" style={{ color: 'var(--text-secondary-color)' }}>Load Existing Template</label>
                                    <select onChange={handleLoadTemplate} 
                                        className="w-full p-2 border rounded-lg text-sm cursor-pointer hover:bg-black/30 transition"
                                        style={{
                    ...
                    <div className={`h-full overflow-hidden bg-slate-200/90 rounded-xl border border-white/10 flex flex-col lg:col-span-8 shadow-inner ${mobileTab === 'preview' ? 'flex' : 'hidden lg:flex'}`}>
                        <div className="flex-grow overflow-auto custom-scrollbar p-4 md:p-8 flex justify-center items-start">
                            <QuotationPreview ref={previewRef} data={{ details, event, lineItems: filteredLineItems }} />
                        </div>
                    </div>
                </div>
            </div>
